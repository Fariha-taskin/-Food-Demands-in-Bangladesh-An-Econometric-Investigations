...
library(psych)
library(ggplot2)
data<-read_dta("G:\\Aniee\\Agriculture\\Data\\2005\\s9a2.dta")
attach(data)
head(data)
glimpse(data)
#data<-rename("hhcode" = "hhold", "foodcode" = "code", "quantity" = "quan", "value" = "valu")
sapply(day, `%in%`, foodcode = 10)
library(tidyverse)
library(forcats)
...
count(fc)
fct_count(fc)
d1<-filter(day %in% c(1,2,3,4,5,6,7))
df1 <- data %>%
...
dat<-append(df1, df2)
length(dat)
write_dta(dat, "G:\\Aniee\\Agriculture\\Data\\2005\\dat.dta", label = attr(dat, "label"))
View(data1)
data11<-rbind(df1,df2)
data11<-rename(data11, week = day)

data2<-read_dta("G:\\Aniee\\Agriculture\\Data\\2005\\s9b.dta")
attach(data2)
...
?zap_labels
data11<-data11[,1:7]
d<-rbind(data11, data22)
tail(d)
glimpse(d)
write.csv(d, "G:\\Aniee\\Agriculture\\Data\\2005\\cn2005.csv")
...
write.csv(data, "G:\\Aniee\\Agriculture\\Final CSV\\data_New2005.csv")

data<- read.csv("G:\\Aniee\\Agriculture\\Final CSV\\data_New2005.csv")
...
write.csv(h_base1, "G:\\Aniee\\Agriculture\\Final CSV\\small_wi_New2005.csv")
write.csv(h_base3, "G:\\Aniee\\Agriculture\\Final CSV\\Index and WI_New2005.csv")
write.csv(h_base4, "G:\\Aniee\\Agriculture\\Final CSV\\Total_Expend_New2005.csv")
